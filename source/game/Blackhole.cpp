...
void Blackhole::Update(float timestep) {
    ...
    if (!m_imploding) {
        ...
        constexpr u32 m_implosionStepLength = 10;
...
void Blackhole::Draw(u32 layerIndex) {
    ...
    else if (m_shootAnimationIdx >= 10) {
        Render::RenderSprite(s_blackhole1Sprite, (s32)m_aabb.pos.x*MAP_PIXEL_ZOOM, (s32)m_aabb.pos.y*MAP_PIXEL_ZOOM, size * 1.55, size * 1.55, m_velocityAngle);
    }
    else {
